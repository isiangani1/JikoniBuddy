            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {orders.filter(o => o.status === col.status).map(order => (
                <div key={order.id} style={{ background: "#211a30", padding: "1rem", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)" }}>
                  ...
                  {col.status !== "ready" ? (
                    <button className="primary" style={{ width: "100%", padding: "0.5rem", marginBottom: "0.5rem" }} onClick={() => advanceStatus(order)}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
                    <button className="ghost" style={{ fontSize: "0.8rem", padding: "0.4rem" }} onClick={() => setActiveChatOrder(order)}>💬 Message</button>
                    {(order.status === 'ready' || order.status === 'completed') && (
                       <button className="ghost" style={{ fontSize: "0.8rem", padding: "0.4rem" }} onClick={() => setActiveTrackingOrder(order)}>📍 Track</button>
                    )}
                  </div>
                </div>
              ))}
         ...
         <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "350px", background: "#1a1026", borderLeft: "1px solid rgba(255,255,255,0.1)", zIndex: 1000, display: "flex", flexDirection: "column", animation: "slideIn 0.3s ease-out" }}>
           ...
           <div style={{ padding: "1.5rem", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
             <div style={{ display: "flex", gap: "0.5rem" }}>
               ...
               />
               <button className="primary" style={{ padding: "0 1rem" }}>Send</button>
             </div>
           </div>
         </div>
       )}
